...
export default function VerifySignupPage() {
  ...
  const [verifySignupOtp, { isLoading: isVerifying }] =
    useVerifySignupOtpMutation();
  const [resendSignupOtp, { isLoading: isResendingOtp }] =
    useResendSignupOtpMutation();
  ...
  const handleResend = async () => {
    ...
    try {
      const result = await resendSignupOtp({ email: emailFromQuery }).unwrap();

      if (result.success) {
        setResendMessage(
          result.message || "New verification OTP sent to your email",
        );
